...
case class REPLesent(width: Int = 0, height: Int = 0, input: String = "REPLesent.txt") {
  ...
  private case class Deck(slides: IndexedSeq[Slide]) {
    ...
    def currentSlide: Slide = slides(slideCursor)

    def currentBuild: Option[Build] = select(slideCursor, buildCursor)
  ...
  def r: Unit = show(deck.currentBuild)
  def refresh: Unit = r

  def reload: REPLesent = copy()
}
